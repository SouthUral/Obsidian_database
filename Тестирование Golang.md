...
```
Затем в цикле обойдем все тестовые случаи из среза `data` вызывая на каждый элемент метод ==`Run`==. В метод  ==`Run`== мы передаем два параметра:
 - имя подтеста;
 - анонимную функцию с одним аргументом ==`func(t *testing.T) {}`==.
Внутри функции анонимной функции  ==`func(t *testing.T) {}`== мы вызываем  функцию, которую мы хотим протестировать `DoMath`, передавая ей поля текущего элемента среза. Этот подход позволяет многократно использовать одну и ту же логику тестов, и, так же, снабдить подтесты именем ==_можно посмотреть через флаг -v_==. 
```Go
for _, d := range data {
...
```
>[!warning] Важно
>Здесь производится сравнение текста ошибок - это не совсем верный подход, т.к. не обязательно текст ошибок может и должен совпадать. Лучше использовать пользовательские типы ошибок и проверять их с помощью `error.Is` или `error.As`.

### Степень покрытия кода
Команда запускает тесты и выводит информацию о покрытии тестами кода.
```shell
go test -cover
```

Команда записывает информацию о тестах и покрытии в файл c.out.
```shell
go test -cover -coverprofile=c.out
```
Теперь, используя этот файл, можно создать и посмотреть в браузере html страницу, в которой будет видно какие сценарии покрыты кодом а какие нет.
```shell
go tool cover -html=c.out
```
После выполнения команды откроется страница в браузере.
![[Pasted image 20231019152521.png]]
Здесь выводится тестируемый код, __зеленым__ отмечен код, который покрыт тестами, __красным__ отмечен непротестированный код.
>[!warning]
>Сведения о покрытии кода - необходимая, но не достаточная информация. Даже в 100% -ном покрытом тестами коде могут быть ошибки.
